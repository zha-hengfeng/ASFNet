...
class CamVidDataSet(data.Dataset):
    ...
    def __init__(self, root='', list_path='', max_iters=None, crop_size=(360, 360),
        ...
        for name in self.img_ids:
            ...
            label_file = label_file.split('.')[0] + '_P_P.png'
            print(label_file)
            self.files.append({
                "img": img_file,
                "label": label_file,
    ...
    def __getitem__(self, index):
        ...
        #print("img:", datafiles['img'])
        #print("label:", datafiles['label'])
...
if __name__ == '__main__':
    # dataset/camvid/image/0001TP_006690.png
    # ./dataset/camvid\image/0016E5_08280.png
    # img = cv2.imread("D:/PyCharmProjects/EACNet-master/dataset/camvid\image/0006R0_f01710.png", cv2.IMREAD_COLOR)
    # label = cv2.imread("D:/PyCharmProjects/EACNet-master/dataset/camvid\labels/0016E5_06270.png", cv2.IMREAD_GRAYSCALE)
    # print(label.shape)
    # print(label)
    # cv2.imshow('lab', label)
    # cv2.waitKey(0)

    list_path = "D:\PyCharmProjects\DABNet-master\dataset\camvid\camvid_train_list.txt"
    img_ids = [i_id.strip() for i_id in open(list_path)]
    for name in img_ids:
        # img_file = osp.join(self.root, name.split()[0])
        # print(img_file)
        # label_file = osp.join(self.root, name.split()[1])
        label_file = label_file.split('.')[0] + '_P.png'
