...
class APF_Moudle(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, M=2, r=16, L=32):
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv2d(out_channels*3, out_channels, 1, 1, padding=1, dilation=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
...
class APFNet(nn.Module):
    def __init__(self, num_classes=19, backbone='res18', encoder_only=True, block_channel=32, use3x3=False):
        ...
        self.apf_m = APF_Moudle(in_channels=2*block_channel, out_channels=64, stride=1, M=2, r=16,
                                      L=num_classes)
